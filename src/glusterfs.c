...
static int cmd_sent;
static int cmd_done;
// standard collectd IgnoreSelected config thingy
// see: https://collectd.org/documentation/manpages/collectd.conf.5.shtml#ignorelists
...
cli_cmd_await_response (unsigned time)
{
        ...
        if (!cmd_done) {
                if (ret == ETIMEDOUT)
                        ERROR("glusterfs plugin: RPC request timed out");
                else
                        ERROR("glusterfs plugin: RPC command returned with error code:%d", ret);
        }
        cmd_done = 0;
...
cli_cmd_broadcast_response (int32_t status)
{
        ...
        pthread_mutex_lock (&cond_mutex);
        {
                ...
                cmd_done = 1;
                rpc_ret = status;
                INFO("glusterfs plugin: received response %d", status);
...
cli_cmd_broadcast_connected ()
{
        ...
        pthread_mutex_unlock (&conn_mutex);
        INFO("glusterfs plugin: Connected to gluster");
...
glusterfs_ctx_defaults_init ()
{
        ...
        if (ret != 0) {
            ERROR("glusterfs plugin: ret = xlator_mem_acct_init (THIS, mt_end);");
                return ret;
        }
                    ...
                    xlator_t *xlator, fop_cbk_fn_t cbkfn, xdrproc_t xdrproc)
{
        ...
        if (req) {
                ...
                if (ret == -1) {
                    ERROR("glusterfs plugin: failed to serialize");
                        goto out;
                }
                iov.iov_len = ret;
                count = 1;
        ...
        if (ret) {
            ERROR("glusterfs plugin: not connected");
        }
        /* Send the msg */
        INFO("glusterfs plugin: sending %d", procnum);
                ...
                void *data)
{
        ...
        case RPC_CLNT_CONNECT:
        {
                ...
                cli_cmd_broadcast_connected ();
                INFO("glusterfs plugin: got RPC_CLNT_CONNECT");
               break;
        }

        case RPC_CLNT_DISCONNECT:
        {
                INFO("glusterfs plugin: got RPC_CLNT_DISCONNECT");
                connected = 0;
        ...
        default:
                INFO("glusterfs plugin: got some other RPC event %d", event);
                ret = 0;
                break;
        }
...
cli_rpc_init (struct cli_state* state)
{
        ...
        if (state->glusterd_sock) {
                NOTICE("glusterfs plugin: Connecting to glusterd using "
                ...
                NOTICE("glusterfs plugin: Connecting to remote glusterd at "
                        "%s", state->remote_host);
...
/*#endif*/
        ...
        INFO("glusterfs plugin: %s", xlator->name);

        ret = rpc_clnt_register_notify (rpc, cli_rpc_notify, xlator);
        if (ret) {
                ERROR("glusterfs plugin: failed to register notify");
                goto out;
        }

        ret = rpc_clnt_start (rpc);
        INFO("glusterfs plugin: client started");
out:
        if (ret) {
            ERROR("glusterfs plugin: error");
                if (rpc)
                        rpc_clnt_unref (rpc);
                rpc = NULL;
        }
...
int process_data2 (dict_t *dict, int count, const char* host) {
    ...
    snprintf (key, sizeof (key), "%d-cumulative", count);
    ret = dict_get_int32 (dict, key, &cumulative);

    memset (key, 0, sizeof (key));
    snprintf (key, sizeof (key), "%d-interval", count);
    ret = dict_get_int32 (dict, key, &interval);

    memset (key, 0, sizeof (key));
    snprintf (key, sizeof (key), "%d-%d-total-read", count, cumulative);
    ...
    memset (key, 0, sizeof (key));
    snprintf (key, sizeof (key), "%d-%d-total-write", count, cumulative);
    ...
    for (i = 0; i < GF_FOP_MAXVALUE; i++) {
        ...
        if (ignorelist_match(ignorelist, gf_fop_list[i]) != 0) {
            continue;
        }
    ...
    INFO("glusterfs plugin: dispatched!");
    return 0;
}
                              ...
                              int count, void *myframe)
{
        ...
        INFO("glusterfs plugin: received response code %d", req->rpc_status);
        if (-1 == req->rpc_status) {
                goto out;
        }

        INFO("glusterfs plugin: Received resp to profile");
        ret = xdr_to_generic (*iov, &rsp, (xdrproc_t)xdr_gf_cli_rsp);
        if (ret < 0) {
                ERROR("glusterfs plugin: Failed to decode xdr response");
                goto out;
        }
        ...
        if (ret) {
                ERROR("glusterfs plugin: Unable to allocate memory");
                goto out;
        } else {
        ...
        INFO("glusterfs plugin: here");
        if (rsp.op_ret && strcmp (rsp.op_errstr, "")) {
                ERROR ("glusterfs plugin: %s", rsp.op_errstr);
        }

        switch (op) {
        case GF_CLI_STATS_START:
                INFO ("glusterfs plugin: Starting volume profile on %s has been %s ",
                         volname,
        ...
        case GF_CLI_STATS_STOP:
                INFO ("glusterfs plugin: Stopping volume profile on %s has been %s ",
                         volname,
                         (rsp.op_ret) ? "unsuccessful": "successful");
                if (!rsp.op_ret) {
                    started = 0;
        ...
        case GF_CLI_STATS_INFO:
                INFO ("glusterfs plugin: volume profile info on %s has been %s ",
                         volname,
                         (rsp.op_ret) ? "unsuccessful": "successful");
                if (rsp.op_ret) {
                    started = 0;
                ...
                break;
        default:
                INFO ("glusterfs plugin: unknown operation for volume profile");
                break;
        }
        ...
        int i = 1;
        char* host = calloc(HOST_NAME_MAX, sizeof(char));
        char* host2 = calloc(HOST_NAME_MAX, sizeof(char));
        struct addrinfo * res;
        while (i <= brick_count) {
                memset (key, 0, sizeof (key));
                ...
                if (ret) {
                        ERROR("glusterfs plugin: Couldn't get brick name using number: %d", i);
                        goto out;
                }
                ...
                host = strtok(brick, ":");
                int ret3 = getaddrinfo(host, NULL, NULL, &res);
                if (ret3)
                    ERROR("glusterfs plugin: error getting host addr info");

                int ret2 = getnameinfo(res->ai_addr, res->ai_addrlen, host2, sizeof(char) * HOST_NAME_MAX, NULL, 0, NI_NAMEREQD);
                if (ret2) 
                    ERROR("glusterfs plugin: error getting hostname");


                INFO("glusterfs plugin: hostname: %s", host2);
                ...
                if (GF_CLI_INFO_CLEAR == info_op) {
                        ...
                        if (ret)
                                goto out;
                        if (stats_cleared) {
                            INFO("glusterfs plugin: Cleared stats.");
                        } else {
                            ERROR("glusterfs plugin: Failed to clear stats.");
                        }
                } else {
                    process_data2(dict, i, host2);
                        /*snprintf (key, sizeof (key), "%d-cumulative", state.bricknum);*/
                        /*ret = dict_get_int32 (dict, key, &interval);*/

                        /*if (ret != 0) {*/
                            /*ERROR("glusterfs plugin: Error occurred in parsing return");*/
                            /*goto out;*/
                        /*}*/
...
void * profiler_thing (void* d) {
    int ret = 0;
        ...
        ret = dict_set_str (dict, "volname", state.volume);
        if (ret)
                goto out;
        ...
        if (started) {
            ...
            ret = dict_set_int32 (dict, "op", (int32_t)GF_CLI_STATS_START);
        }
        if (ret)
            goto out;
        ...
        if (!cli_ret) {
            INFO("glusterfs plugin: awaiting response");
            cmd_sent = 1;
            ret = cli_cmd_await_response(600);
        }
        pthread_mutex_unlock (&cond_mutex);

        INFO("glusterfs plugin: Returning %d", ret);
...
out:
        INFO("glusterfs plugin: Exiting with: %d", ret);
...
void* start_loop(void* d)
{
    ...
    /*if (!state.ctx) {*/
        ...
        if (!ctx) {
                ERROR("glusterfs plugin: glusterfs_ctx_new: %d", ENOMEM);
                return NULL;
        }
/*#ifdef DEBUG*/
        /*ERROR("glusterfs plugin: Setting debug");*/
        /*gf_mem_acct_enable_set (ctx);*/
/*#endif*/

        ret = glusterfs_globals_init (ctx);
        if (ret) {
                ERROR("glusterfs plugin: glusterfs_globals_init: %d", ret);
                return NULL;
        }
        ...
        if (ret) {
            ERROR("glusterfs plugin: ERROR SETTING CTX DEFAULTS: %d", ret);
            return NULL;
        }
...
/*#endif*/
        ...
        if (!global_rpc) {
            ERROR("glusterfs plugin: global_rpc is null");
            return NULL;
        }

        /*ret = pthread_create ((&(state.input)), NULL, profiler_thing, &state);*/
        /*if (ret) {*/
            /*ERROR("glusterfs plugin: problem! %d", ret);*/
            /*return NULL;*/
        /*}*/
...
static int read_stats(void) {
    ...
    INFO("glusterfs plugin: starting read");
    pthread_mutex_lock (&interval_mutex);
    {
